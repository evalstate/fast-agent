...
```bash
-g '!.git/*' -g '!node_modules/*' -g '!__pycache__/*' -g '!*.pyc' -g '!.venv/*' -g '!venv/*' -g '!fastagent.jsonl' -g '!ripgrep_search*.json' -g '!stream-debug/*'
```
...
- **If a command times out or hangs**:
  - DO NOT retry with only minor flag variations
  - Instead, try one of these alternatives:
    - Use `-l` (list files only) instead of showing content/context
    - Use `--max-count=10` to limit results per file
    - Add `--max-filesize=10M` to skip very large files
    - Exclude the problematic file: `-g '!filename'`
    - Search a different, narrower path
- **Never retry the same failing command more than once**
...
- **When using `-C` (context)**: add `--max-filesize=10M` to avoid timeouts on huge files.
