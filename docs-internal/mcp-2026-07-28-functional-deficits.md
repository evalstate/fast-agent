...
| MCP2-007 | MCP OpenTelemetry auto-instrumentation | MCP-specific third-party auto-instrumentation is not installed or activated. | The latest compatible candidate, `opentelemetry-instrumentation-mcp==0.62.1`, patches the removed SDK v1 symbol `mcp.client.streamable_http.streamablehttp_client` and crashes telemetry setup with SDK v2. | Re-add and enable `McpInstrumentor` when an SDK-v2-compatible release is available; fast-agent's own MCP progress and timing telemetry remains active. |
...
| MCP3-006 | FastMCP elicitation examples | FastMCP v4 `Context.elicit()` still uses a standalone server-to-client request, which SDK v2 rejects on a negotiated `2026-07-28` connection. Three existing custom-handler integration cases therefore fail even though the SDK-native MRTR path succeeds. | FastMCP `4.0.0b1` can represent manual input-required guards but does not expose the SDK `MCPServer` resolver/`Resolve(...)` API; its imperative context helper still uses the standalone path. | Migrate the examples to SDK `MCPServer` + `Resolve(Elicit(...))`, or adopt the FastMCP MRTR API when one is public. |
...
| MCP2-002 | Legacy `Mcp-Session-Id` diagnostics are recovered from the public HTTP client's response event hook without depending on removed SDK transport internals. |
...
| MCP3-008 | MCP Python SDK `2.0.0` supports `subscriptions/listen` over modern stdio connections; the supervisor uses the same subscription lifecycle for modern HTTP and stdio transports. |
