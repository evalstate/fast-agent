...
from fast_agent.core.agent_card_function_tool_validation import (
    check_function_tool_spec,
    ensure_function_tool_list,
    iter_function_tool_specs,
)
from fast_agent.core.agent_card_mcp_connect_validation import validate_mcp_connect_entries
...
from fast_agent.tools.function_tool_config import FunctionToolSpec, function_tool_entrypoint
...
def _scan_single_agent_card_file(card_path: Path) -> _ScannedCardDetails:
    ...
    validate_mcp_connect_entries(raw.get("mcp_connect"), errors)
    function_tools = ensure_function_tool_list(raw.get("function_tools"), "function_tools", errors)
    ...
    if details.function_tools:
        ...
        for spec in details.function_tools:
            ...
            if entrypoint is None:
                continue
            error = check_function_tool_spec(entrypoint, base_path)
            if error:
                errors.append(error)
    ...
    while True:
        ...
        for name in sorted(remaining):
            ...
            if config is not None and config.function_tools:
                ...
                for spec in iter_function_tool_specs(config.function_tools):
                    error = check_function_tool_spec(spec, base_path)
                    if error:
                        issues.append(
                            LoadedAgentIssue(
                                name=name,
