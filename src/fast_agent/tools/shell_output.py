...
def process_output_preview(blob: bytes, *, limit: int, total_bytes: int) -> str:
    ...
    if shown < total_bytes:
        text += (
            f"\n[Output truncated: showing {shown} of {total_bytes} bytes. "
    ...
    def consume(self, limit: int | None = None) -> str:
        if limit is None:
            ...
            combined_output = process_output_preview(
                blob, limit=limit, total_bytes=self.total_output_bytes
            )
