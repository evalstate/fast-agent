"""Compatibility copy of FastMCP's Hugging Face OAuth provider.

This module mirrors the native FastMCP provider shape so fast-agent can switch
to ``fastmcp.server.auth.providers.huggingface`` when the installed FastMCP
version includes it.
"""
...
def _extract_scopes(data: Mapping[str, Any]) -> list[str]:
    ...
    if isinstance(nested_scopes, list):
        ...
        for scope in nested_scopes:
            raw = scope.get("name") if isinstance(scope, Mapping) else scope
