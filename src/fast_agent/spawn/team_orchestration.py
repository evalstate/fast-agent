"""Team Orchestration — workflow-specific logic for team spawns.

Contains agile-specific orchestration patterns such as review loops,
verdict parsing, meeting orchestration, and retrospectives. These are
separated from the core team_spawner primitives so that host
applications can replace them with custom implementations.

All functions here import primitives from ``fast_agent.spawn.team_spawner``.
"""
...
from fast_agent.spawn.workspace_manager import append_changelog
...
def parse_verdict(review_text: str) -> dict[str, str]:
...
async def run_review_loop(
    ...
    from fast_agent.spawn.team_spawner import _spawn_step
...
def _audit_meeting(meeting_dir: Path, message: str) -> None:
    ...
    with open(audit_path, "a", encoding="utf-8") as f:  # noqa: SIM115
