"""Startup model resolution and picker helpers for CLI runtime requests."""
...
def is_interactive_startup_notice_context(request: AgentRunRequest) -> bool:
...
def should_prompt_for_model_picker(
    ...
    *,
    ...
    if not is_interactive_startup_notice_context(request):
        return False
...
async def select_startup_model_if_needed(request: AgentRunRequest) -> str | None:
    ...
    settings = load_request_settings(request)
    can_prompt_for_model = should_prompt_for_model_picker(
        request,
        stdin_is_tty=sys.stdin.isatty(),
        stdout_is_tty=sys.stdout.isatty(),
    )
    startup_model_defined_by_card = explicit_agent_cards_define_startup_model(
        request,
        model_references=settings.model_references,
        system_default_requires_explicit=(
            can_prompt_for_model
            and system_default_reference_is_missing(settings_model_references(settings))
        ),
    )
    resolved_model = resolve_model_without_hardcoded_default(
        model=request.model,
    ...
    if (
        ...
        and should_prompt_for_unpinned_system_default(settings, can_prompt=can_prompt_for_model)
    ):
        initial_selection = resolve_model_picker_initial_selection(settings=settings)
        request.model = await select_model_from_picker(
            request,
