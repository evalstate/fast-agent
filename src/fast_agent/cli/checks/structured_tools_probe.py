...
async def _probe_model(
    ...
    *,
    ...
    try:
        response = await agent.generate(_build_prompt(), request_params=request_params)
        resolved_model = agent.llm.resolved_model if agent.llm is not None else None
        ...
        provider = agent.llm.provider.config_name if agent.llm is not None else None
        stop_reason = response.stop_reason.value if response.stop_reason is not None else None
        return ProbeResult(
            model=model,
            resolved_model=resolved_model.wire_model_name if resolved_model is not None else None,
            provider=provider,
            json_mode=resolved_model.json_mode if resolved_model is not None else None,
            structured_tool_policy=structured_tool_policy,
            passed=True,
...
def _print_text_summary(results: list[ProbeResult]) -> None:
    for result in results:
        ...
        details = f"tool_calls={result.tool_calls} stop_reason={result.stop_reason or '-'}"
