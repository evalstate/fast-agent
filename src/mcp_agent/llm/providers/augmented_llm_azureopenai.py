...
from openai import AuthenticationError, OpenAI, AzureOpenAI
...
class AzureOpenAIAugmentedLLM(AugmentedLLM[ChatCompletionMessageParam, ChatCompletionMessage]):
    ...
    def __init__(self, provider_name: str = "AzureOpenAI", *args, **kwargs) -> None:
        ...
        self._reasoning = chosen_model and (
            chosen_model.startswith("azure-o3") or chosen_model.startswith("azure-o1")
        )
        if self._reasoning:
            self.logger.info(
    ...
    def _api_version(self) -> str:
        return self.context.config.azureopenai.api_version if self.context.config.azureopenai else None
