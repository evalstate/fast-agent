...
fast-agent supports the
[A2A Protocol Specification 1.0](https://a2a-protocol.org/v1.0.0/specification/)
for HTTP transports. fast-agent uses the `a2a-sdk` 1.0 server and client stack
and does not expose a gRPC binding.
...
| Standalone refinement messages | Supported for A2A-native/tool-capable served agents | Tool-capable agents defer task creation and can call `return_message` to send an A2A `Message` before deciding to start a task. Ordinary served agents keep the default task-generating behavior. |
...
| Gap | Impact | Behavior |
|---|---|---|
| gRPC transport | Not supported. | The AgentCard does not advertise gRPC, and the CLI/API should use `JSONRPC` or `HTTP+JSON`. |
| Push notifications | Unsupported. | The AgentCard advertises `pushNotifications=false`; SDK push configuration methods return the protocol's not-supported error. Streaming and polling remain available. |
| Extended AgentCard | Unsupported. | The server publishes the public AgentCard only and does not configure `extendedAgentCard`. |
...
| Typed audio content on the server | Limited. | The client can send `AudioContent` as raw A2A parts. The server preserves inbound audio bytes as blob resources rather than mapping them to a dedicated fast-agent `AudioContent` object. |
| Structured JSON output from model text | Limited. | fast-agent JSON text responses remain text artifacts unless represented as `TextResourceContents` with `mime_type="application/json"`. This avoids guessing whether ordinary text is intended to be protocol data. |
...
| Idempotent `messageId` handling | No fast-agent deduplication. | The SDK validates request shape, but fast-agent does not deduplicate repeated `messageId` values. |
| AgentCard signing | Unsupported. | The public AgentCard is unsigned. |
| Extension negotiation | Unsupported. | The server does not advertise or process custom A2A extensions. |

## Verification

The A2A test suite covers:

- JSON-RPC and HTTP+JSON client/server calls;
- `context_id` continuity across turns;
...
- AgentCard skill advertisement and fast-agent-specific routing via message
  metadata;
- standalone A2A refinement messages before task creation;
...
For an end-to-end provider check, run a fast-agent A2A server with a
streaming-capable model and connect to it with the fast-agent A2A client. Stream
listeners should receive incremental text before the final task completion
event.
