...
| `logger.tool_display.arguments` | `LOGGER__TOOL_DISPLAY__ARGUMENTS` | `Literal['auto', 'all', 'none']` | `auto` | Tool argument body visibility. |
...
| `logger.tool_display.aggregate_parallel` | `LOGGER__TOOL_DISPLAY__AGGREGATE_PARALLEL` | `bool` | `True` | Aggregate safe parallel calls to the same generic tool. |
