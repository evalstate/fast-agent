...
| `shell_execution.process_poll_max_wait_seconds` | `SHELL_EXECUTION__PROCESS_POLL_MAX_WAIT_SECONDS` | `int` | `260` | Maximum managed-process wait. |
| `shell_execution.foreground_auto_await_max_seconds` | `SHELL_EXECUTION__FOREGROUND_AUTO_AWAIT_MAX_SECONDS` | `int` | `30` | Maximum total foreground runtime before returning a live process. |
