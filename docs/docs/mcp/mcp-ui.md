...
To connect to the demo server with `gpt-5-mini` and low reasoning effort, use:

```bash
fast-agent --url http://localhost:3000 --model "gpt-5-mini?reasoning=low"
```
...
fast-agent --url http://localhost:3000 --model "gpt-5-mini?reasoning=low",sonnet
```
...
If you want to pass authorization headers, use the `--auth` option:
...
Some MCP Servers adjust their tools or behaviour based on the connecting client, for example by enabling mcp-ui. You can specify the name and version to present to the MCP Server:
