...
cat > "$HOME_DIR/fastagent.config.yaml" <<YAML
...
echo "[cards-smoke] 1/4 add using --home config (no --registry)"
"$FAST_AGENT_BIN" --home "$HOME_DIR" cards add alpha >/dev/null

test -f "$HOME_DIR/agent-cards/alpha.md"
echo "[cards-smoke]      OK: installed via cards.marketplace_urls in home config"

echo "[cards-smoke] 2/4 remove"
"$FAST_AGENT_BIN" --home "$HOME_DIR" cards remove alpha >/dev/null
...
if "$FAST_AGENT_BIN" --home "$HOME_DIR" cards add alpha >/dev/null 2>&1; then
...
"$FAST_AGENT_BIN" --home "$HOME_DIR" cards --registry "$MARKETPLACE" add alpha >/dev/null

test -f "$HOME_DIR/agent-cards/alpha.md"
echo "[cards-smoke]      OK: --registry override works"

echo "✅ cards CLI home smoke passed"
if [[ "$KEEP_TMP" == "1" ]]; then
