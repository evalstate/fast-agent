"""
Tests for server disconnect handling and reconnection functionality.
...
class TestServerSessionTerminatedError:
    ...
    def test_error_code_constant(self):
        """Test that the SESSION_TERMINATED_CODE constant is defined."""
...
class TestSessionTerminationDetection:
    ...
    def test_detection_with_error_without_code(self):
        """Test that errors without a code attribute are not detected."""
        from mcp.shared.exceptions import McpError
        from mcp.types import ErrorData

        from fast_agent.mcp.mcp_agent_client_session import MCPAgentClientSession

        # Create a McpError without a code (using 0 which is different from -32600)
