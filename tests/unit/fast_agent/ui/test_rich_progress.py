...
class TestAggregatorInitializedVisibility:
    ...
    def test_process_output_progress_refreshes_live_poll_baselines(self) -> None:
        ...
        assert (
            rendered.plain
            == "out <1s · err —   · time 1m10s · size 25.0KB · uv run worker.py"
        )
    ...
    def test_process_output_activity_fades_then_goes_quiet(self) -> None:
        ...
        assert warm.plain == "out 12s · err —   · time 1m30s · size 12.5KB"
        ...
        assert quiet.plain == "out 1m30s · err —   · time 1m30s · size 12.5KB"
        display.stop()
