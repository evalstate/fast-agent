...
from fast_agent.utils.async_utils import (
    ...
    _suppress_known_uvloop_event_loop_policy_deprecations,
...
def test_suppress_known_uvloop_event_loop_policy_deprecations_installs_filters() -> None:
    with warnings.catch_warnings():
        warnings.resetwarnings()
        _suppress_known_uvloop_event_loop_policy_deprecations(version_info=(3, 14))
        ...
        assert _ASYNCIO_SET_EVENT_LOOP_POLICY_DEPRECATION_MESSAGE in patterns
        assert _ASYNCIO_ABSTRACT_EVENT_LOOP_POLICY_DEPRECATION_MESSAGE in patterns
