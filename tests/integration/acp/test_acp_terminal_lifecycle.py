...
async def test_terminal_create_lifecycle() -> None:
    ...
    result1 = await client.terminal_create({"sessionId": session_id, "command": "echo hello"})
    ...
    result2 = await client.terminal_create({"sessionId": session_id, "command": "pwd"})
    ...
    await client.terminal_release({"sessionId": session_id, "terminalId": terminal_id1})
    ...
    await client.terminal_release({"sessionId": session_id, "terminalId": terminal_id2})
    assert len(client.terminals) == 0
...
async def test_terminal_output_retrieval() -> None:
    ...
    # Create terminal
    result = await client.terminal_create({"sessionId": session_id, "command": "echo test output"})
    terminal_id = result["terminalId"]

    # Get output
    output = await client.terminal_output({"sessionId": session_id, "terminalId": terminal_id})
    ...
    # Cleanup
    await client.terminal_release({"sessionId": session_id, "terminalId": terminal_id})
...
async def test_terminal_wait_for_exit() -> None:
    ...
    # Create terminal
    result = await client.terminal_create({"sessionId": session_id, "command": "echo test"})
    terminal_id = result["terminalId"]

    # Wait for exit (immediate in test client)
    exit_result = await client.terminal_wait_for_exit({"sessionId": session_id, "terminalId": terminal_id})
...
async def test_terminal_kill() -> None:
    ...
    # Create terminal
    result = await client.terminal_create({"sessionId": session_id, "command": "sleep 100"})
    terminal_id = result["terminalId"]

    # Kill it
    await client.terminal_kill({"sessionId": session_id, "terminalId": terminal_id})
...
async def test_terminal_release_cleanup() -> None:
    ...
    terminals = []
    for i in range(3):
        result = await client.terminal_create({"sessionId": session_id, "command": f"echo {i}"})
    ...
    await client.terminal_release({"sessionId": session_id, "terminalId": "nonexistent"})
...
async def test_terminal_missing_id() -> None:
    ...
    output = await client.terminal_output({"sessionId": session_id, "terminalId": "missing"})
    assert output["output"] == ""
    assert output["exitCode"] is None

    # Wait for non-existent terminal
    exit_result = await client.terminal_wait_for_exit({"sessionId": session_id, "terminalId": "missing"})
    assert exit_result["exitCode"] == -1

    # Kill non-existent terminal (should not error)
    await client.terminal_kill({"sessionId": session_id, "terminalId": "missing"})
